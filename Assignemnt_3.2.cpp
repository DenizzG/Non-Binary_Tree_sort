    ...
    for (int i = 1; i < matrix.size(); i++) {
        ...
        if (i+1 < matrix.size() && matrix[i][column_n] == matrix[i+1][column_n]){
            ...
            if (it != attribute_map.end()) {
                ...
                attribute_map[matrix[i][column_n]] = 1;
            }
        }
...
std::cout << column_n <<"JDFIJBDSIGFJS" <<std::endl;
...
public:
    ...
    std::cout << std::endl;
        ...
        std::vector<double> entropy_column_chosen = entropy_of_column(matrix, minIndex);
         ...
         for (int i = 0; i < entropy_column_chosen.size(); ++i) {
                std::cout << entropy_column_chosen[i] << " ";
            }
            std::cout << std::endl;
...
int main(){
    ...
    t.printTree(t.return_root());
     ...
     std::vector<std::string> q =  {"a13480", "5", "4523", "a"};
     //std::cout <<"Printing: " << t2.query(q) << std::endl;
